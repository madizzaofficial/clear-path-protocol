...
function Suivi() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      checkins28Snap?.forEach((d: any) => {
        checkins28[d.id] = d.data();
      });
      ...
      setData({
        ...
        completedLessons: progressSnap?.exists()
          ? (progressSnap.data().completedLessons ?? [])
          : [],
        enrolledAt: userSnap?.exists()
          ? (userSnap.data().routineStartedAt ?? userSnap.data().enrolledAt ?? null)
          : null,
        totalRoutineSteps,
        phases,
        amSteps,
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start">
            ...
            <div className="rounded-3xl border border-border/60 bg-card p-7 shadow-soft lg:flex-[7]">
              ...
              <p className="mt-3 leading-relaxed text-muted-foreground">
                Ton questionnaire a bien été reçu. Je suis actuellement en train d'analyser ton
                profil afin de construire une routine adaptée à ta peau.
              </p>
                ...
                {PENDING_STEPS.map((s, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-3">
                      ...
                      <p
                        className={`text-sm ${s.done ? "font-medium" : isActive ? "font-medium text-primary" : "text-muted-foreground"}`}
                      >
                        {s.label}
                      </p>
                    </div>
          ...
          <div className="space-y-4 lg:space-y-0 lg:grid lg:grid-cols-3 lg:gap-6">
            ...
            <div className="overflow-hidden rounded-3xl bg-gradient-warm p-6 shadow-elegant lg:col-start-1 lg:row-start-1 lg:flex lg:flex-col">
              ...
              <div className="mt-4 flex-1">
                <p className="text-base font-semibold">
                  {currentPhase.title || phaseLabel(currentPhase.fromWeek, currentPhase.toWeek)}
                </p>
              ...
              <div className="flex flex-1 items-center">
                {hasSkinMetrics ? (
                  (() => {
                    ...
                    const infDesc =
                      infPct >= 67 ? "Active" : infPct >= 34 ? "Modérée" : "Sous contrôle";
                    const barDesc =
                      barPct >= 67 ? "Compromise" : barPct >= 34 ? "En cours" : "Excellente";
                    const acnDesc =
                      acnPct >= 67 ? "Active" : acnPct >= 34 ? "Modérée" : "Contrôlée";
                    return (
                  ...
                  <div className="flex items-start gap-3 py-2">
                    ...
                    <div>
                      <p className="text-sm text-muted-foreground">
                        Ton coach met à jour ton bilan de peau.
                      </p>
            ...
            <div className="flex flex-col gap-4 lg:col-start-3 lg:row-start-2 lg:row-span-3 lg:gap-6">
              ...
              <div className="rounded-3xl border border-border/60 bg-card p-5 shadow-soft">
                ...
                {skinType || (acneTypes && acneTypes.length > 0) || intensity ? (
                  ...
                  <p className="text-sm text-muted-foreground">
                    Ton profil sera renseigné après ton bilan d'intake.
                  </p>
                )}
                <Link
                  to="/profile"
              ...
              <div className="rounded-3xl border border-border/60 bg-card p-5 shadow-soft">
                ...
                {totalRoutineSteps > 0 ? (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span
                            className={`text-xs font-semibold tabular-nums ${amDone >= amSteps ? "text-emerald-600 dark:text-emerald-400" : "text-muted-foreground"}`}
                          >
                            {amDone}/{amSteps}
                          </span>
                        </div>
                        <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-muted">
                          <div
                            ...
                            style={{
                              width:
                                amSteps > 0 ? `${Math.min((amDone / amSteps) * 100, 100)}%` : "0%",
                            }}
                          />
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span
                            className={`text-xs font-semibold tabular-nums ${pmDone >= pmSteps ? "text-emerald-600 dark:text-emerald-400" : "text-muted-foreground"}`}
                          >
                            {pmDone}/{pmSteps}
                          </span>
                        </div>
                        <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-muted">
                          <div
                            ...
                            style={{
                              width:
                                pmSteps > 0 ? `${Math.min((pmDone / pmSteps) * 100, 100)}%` : "0%",
                            }}
                          />
                        </div>
                      </div>
                  ...
                  <div className="flex items-start gap-3 py-1">
                    ...
                    <div>
                      <p className="text-sm text-muted-foreground">
                        Ta routine personnalisée sera bientôt disponible.
                      </p>
                      <p className="mt-0.5 text-xs text-muted-foreground/50">
                        Ton coach la prépare pour toi.
                      </p>
                    </div>
                  </div>
                )}
            ...
            <div className="rounded-3xl border border-border/60 bg-card p-5 shadow-soft lg:col-span-2 lg:row-start-2 lg:self-start">
              ...
              <div className="space-y-1">
                {journeyRanges.map(({ phase, dayStart, dayEnd }, i) => {
                  const isPast = dayCount > dayEnd;
                  const isCurrent = dayCount >= dayStart && dayCount <= dayEnd;
                  return (
                    <div
                      key={phase.id}
                    ...
                    >
                      ...
                      <div
                        className={`min-w-0 flex-1 ${isPast ? "opacity-50" : !isCurrent ? "opacity-35" : ""}`}
                      >
              ...
              {photos.length > 0 ? (
                ...
                <div className="flex flex-col items-center gap-3 py-4 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Prends une photo chaque semaine pour suivre ta transformation.
                  </p>
                  <Link
                    to="/journal"
